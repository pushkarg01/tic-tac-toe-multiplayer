...
public class Register : MonoBehaviour
{
    ...
    [SerializeField] CreateAndJoinRoom _createAndJoin;
    string _userName = "";
    ...
    private void Start()
    {
        ...
        _createAndJoin.gameObject.SetActive(false);
    }
    ...
    private void OnClickedSubmitBtn()
    {
        ...
        _apiHandler.RegisterUser(_userName, (Status, data) =>
        {
            if (Status)
            {
                _apiHandler.CheckUserExistence(UniversalConstants.AuthID, (status, userData) => 
                {
                    if (status)
                    {
                        UniversalConstants.UserName = userData.userName;
                        Debug.Log("User Registered and Logged In: " + UniversalConstants.UserName);

                        this.gameObject.SetActive(false);
                        _createAndJoin.gameObject.SetActive(true);
                    }
                });
            ...
            {
                _messageBox.UpdateMessage("Registration failed. Please try again.");
                return;
            }
        });
    }
    ...
    private void OnAlreadyLogin()
    {
        ...
        {
            if (Status && PlayerPrefs.GetInt(UserPrefs.IsLoggedIn)==1)
            {
                UniversalConstants.UserName = data.userName;
    ...
    {
        _usernameInput.onValueChanged.RemoveListener(OnvalueChangeInUserName);
        _submitBtn.onClick.RemoveListener(OnClickedSubmitBtn);
    }
}
